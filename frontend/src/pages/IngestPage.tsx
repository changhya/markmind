...
const stepIcon: Record<StepState, React.ReactNode> = {
  wait:    <span className="w-4 h-4 rounded-full border border-slate-600 inline-block" />,
...
const stepLabel: Record<StepState, string> = {
  wait: 'text-slate-600', running: 'text-indigo-300', done: 'text-emerald-400', error: 'text-red-400',
};
...
export default function IngestPage() {
  ...
  return (
    <div className="flex flex-col h-full p-6 gap-6 overflow-y-auto">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-white">Knowledge Ingest</h1>
        <p className="text-sm text-slate-400 mt-1">문서를 업로드하여 AI 지식 베이스를 구축합니다</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-4">
        <StatCard icon={Files}    label="총 문서"     value={totalCount}           color="bg-indigo-900/50 text-indigo-400" />
        <StatCard icon={Percent}  label="처리 성공률"  value={`${successRate}%`}    color="bg-emerald-900/50 text-emerald-400" />
        <StatCard icon={Database} label="DB 용량"     value={stats ? fmtBytes(stats.db_size_bytes + stats.chroma_size_bytes) : '—'} color="bg-amber-900/50 text-amber-400" />
      </div>
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-colors ${
          isDragActive ? 'border-indigo-500 bg-indigo-900/20' : 'border-slate-700 hover:border-slate-500 hover:bg-slate-800/50'
        }`}
      >
        <input {...getInputProps()} />
        {uploading ? (
          ...
          <Upload className={`w-8 h-8 mx-auto mb-3 ${isDragActive ? 'text-indigo-400' : 'text-slate-500'}`} />
        )}
        <p className="text-sm font-medium text-slate-300">
          {uploading ? '업로드 중...' : isDragActive ? '여기에 파일을 놓으세요' : 'PDF, DOCX, PPTX 파일을 드래그하거나 클릭하세요'}
        </p>
        <p className="text-xs text-slate-500 mt-1">여러 파일 동시 업로드 가능</p>
      </div>

      {/* Document Queue */}
      {docs.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wide">처리 큐</h2>
          {docs.map((doc) => {
            const steps = getSteps(doc);
            const isProcessing = doc.status === 'pending' || doc.status === 'processing';
            return (
              <div key={doc.id} className="bg-slate-800 rounded-xl p-4 border border-slate-700">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <FileText className="w-5 h-5 text-indigo-400 flex-shrink-0" />
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-white truncate">{doc.filename}</p>
                      <p className="text-xs text-slate-400">{fmtBytes(doc.file_size)} · {new Date(doc.created_at).toLocaleString('ko-KR')}</p>
                    </div>
                  </div>
                ...
                {doc.status === 'error' && doc.error_message && (
                  <p className="text-xs text-red-400 mt-2 bg-red-900/20 px-3 py-1.5 rounded-lg">
                    {doc.error_message}
                  </p>
                )}
              </div>
